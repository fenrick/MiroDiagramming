...
This document narrows focus to the **nine sidebar tabs** in the Structured Diagramming add‑on.  
Each tab section specifies panel layout, visible controls, states, interaction flows, tool‑tips, keyboard shortcuts, and validation rules—so any developer can translate designs into code with zero ambiguity.
...
Validation: warn if width/height > 10 000 px (“That’s bigger than your board viewport”).  
...
Flow: Change value → overlay updates real‑time. Press **Arrange** creates frame if enabled.
...
Insert Flow: click → loads JSON → `GraphProcessor` → `BoardBuilder.sync` → opens Rename modal.
...
Error: if PNG > 16 k × 16 k px → show modal “Canvas too large; zoom or frame to export.”
...
2. **Test Connection** – Ping and show latency badge (green < 200 ms, yellow < 500 ms, red otherwise).  
...
* **Editor** – RichTextInput supports `@mention`; autocomplete list uses `miro.board.getUsers()`.  
* **Resolve Toggle** – Checkbox; resolved threads greyed out, filtered when view = “Unresolved”.  
...
1. **File type** – Only .json/.csv/.mmd accepted; else toast “Unsupported file type”.  
2. **Graph cycles** – Detect self‑loops; prompt “Cannot layout self‑loop. Break edge?”  
3. **Selection empty** – Tabs with selection‑based actions show banner: “Select items to enable controls.”  
4. **Network fail (Data Tab)** – Red status badge, retry logic 3× exponential backoff.
...
_Keep this blueprint adjacent to the codebase; update whenever controls or flows change._  

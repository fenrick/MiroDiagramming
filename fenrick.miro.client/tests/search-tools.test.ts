...
describe("search-tools",
  () => {
    ...
    test("tag, colour and assignee filters",
      async () => {
        const { board, items } = makeBoard();
    ...
    test("creator and modifier filters",
      async () => {
        const { board, items } = makeBoard();
    ...
    test("regex and whole-word options",
      async () => {
        const { board, items } = makeBoard();
    ...
    test("case sensitivity and selection only",
      async () => {
        const { board, items } = makeBoard();
    ...
    test("bulk replace preserves formatting",
      async () => {
        const { board, items } = makeBoard();
    ...
    test("onMatch callback is invoked for each replacement",
      async () => {
        const { board, items } = makeBoard();
        const seen: unknown[] = [];
        const count = await replaceBoardContent(
    ...
    test("regex replacement updates all matching patterns",
      async () => {
        const { board, items } = makeBoard();
        const count = await replaceBoardContent(
        ...
        expect(items[0].text).toBe("hi world");
        expect(items[1].title).toBe("hi there");
        expect((items[3].text as { plainText: string }).plainText)
          .toBe("hi test");
        expect(items[4].content).toBe("hi <b>World</b>");
        expect(items[5].plainText).toBe("ahib");
      });

    test("search utilities throw without board",
      async () => {
    ...
    test("replaceBoardContent skips widgets lacking text fields",
      async () => {
        const items = [
    ...
    test("setStringAtPath guards prototype pollution",
      async () => {
    ...
    test("searchBoardContent rejects invalid regex patterns",
      async () => {
    ...
    test("replaceBoardContent rejects invalid regex patterns",
      async () => {
        const { board } = makeBoard();
        await expect(
          replaceBoardContent(
    ...
    test("searchBoardContent rejects unsafe regex patterns",
      async () => {
        const { board } = makeBoard();
        await expect(
    ...
    test("replaceBoardContent rejects excessively long patterns",
      async () => {
        const { board } = makeBoard();

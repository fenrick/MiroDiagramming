...
describe("grid-tools",
  () => {
    ...
    test("applyGridLayout positions widgets",
      async () => {
        const items = [
        ...
        expect(board.group)
          .toHaveBeenCalledWith({ items: [items[1], items[0]] });
        expect(items[0].sync).toHaveBeenCalled();
        expect(items[1].sync).toHaveBeenCalled();
      });

    test("applyGridLayout handles groups as single items",
      async () => {
        const items = [
    ...
    test("applyGridLayout returns early with empty selection",
      async () => {
        const board: BoardLike = {
          getSelection: jest.fn().mockResolvedValue([]),
          group: jest.fn(),
        };
        await applyGridLayout({ cols: 1, padding: 0, groupResult: true },
          board);
        expect(board.group).not.toHaveBeenCalled();
      });

    test("applyGridLayout skips grouping when API missing",
      async () => {
    ...
    test("applyGridLayout sorts by nested text fields",
      async () => {
        const items = [
          {
            x: 0,
    ...
    test("applyGridLayout sorts vertically when requested",
      async () => {
        const items = [
          { x: 0, y: 0, width: 10, height: 10, sync: jest.fn(), title: "b" },
          { x: 0, y: 0, width: 10, height: 10, sync: jest.fn(), title: "a" },
    ...
    test("applyGridLayout handles frames",
      async () => {
        const items = [
    ...
    test("applyGridLayout ignores unsupported items",
      async () => {
        const items = [
          { x: 0, y: 0, width: 10, height: 10, sync: jest.fn() },
          { foo: "bar" },
    ...
    test("applyGridLayout throws without board",
      async () =>

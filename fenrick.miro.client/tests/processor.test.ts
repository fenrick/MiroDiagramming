...
describe("GraphProcessor",
  () => {
    ...
    beforeEach(() => {
      global.miro = {
        board: {
          ...
          createFrame: jest.fn()
            .mockResolvedValue({ add: jest.fn(), id: "f1" }),
          group: jest
            .fn()
            .mockResolvedValue({
              type: "group",
    ...
    it(
      "processGraph runs without throwing and syncs items once after validation",
      async () => {
        const spy = jest
    ...
    it("delegates work to helper methods",
      async () => {
        ...
        const connectorSpy =
          jest.spyOn(gp as unknown, "createConnectorsAndZoom");
    ...
    it("positions frame at space center",
      async () => {
        const simpleGraph = {
          nodes: [{ id: "n1", label: "A", type: "Motivation" }],
          edges: [],
    ...
    it("zooms to shapes when no frame created",
      async () => {
        const simpleGraph = {
          nodes: [{ id: "n1", label: "A", type: "Motivation" }],
          edges: [],
    ...
    it("records widget ids for rows",
      async () => {
        const simpleGraph = {
          nodes: [
            {
    ...
    it("throws when edge source is missing",
      async () => {
        const graph = {
          nodes: [{ id: "n1", label: "A", type: "Motivation" }],
          edges: [{ from: "n2", to: "n1" }],
    ...
    it("throws when edge target is missing",
      async () => {
        const graph = {
          nodes: [{ id: "n1", label: "A", type: "Motivation" }],
          edges: [{ from: "n1", to: "n2" }],

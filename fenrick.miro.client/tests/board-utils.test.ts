...
describe("forEachSelection",
  () => {
    describe("callback invocation",
      () =>
      test("invokes callback once per item",
        async () => {
          const items = [{ a: 1 }, { b: 2 }];
    ...
    describe("error propagation",
      () =>
      test("rejects when callback throws",
        async () => {
...
describe("maybeSync",
  () => {
    test("invokes sync when present",
      async () => {
    ...
    test("resolves when sync missing",
      async () =>
...
describe("getFirstSelection",
  () => {
    test("returns first selected item",
      async () => {
        const items = [{ a: 1 }, { b: 2 }];
        const board = { getSelection: jest.fn().mockResolvedValue(items) };
    ...
    test("returns undefined when selection empty",
      async () => {
        const board = { getSelection: jest.fn().mockResolvedValue([]) };
        const result = await getFirstSelection(board);

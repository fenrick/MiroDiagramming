...
describe("StructuredTab",
  () => {
    test("frame title field toggles with checkbox",
      async () => {
        render(<StructuredTab/>);
        const input = screen.getByTestId("file-input");
        await act(async () =>
          fireEvent.change(input,
            { target: { files: [makeFile("graph.json")] } }),
        );
        expect(
    ...
    test("advanced options reveal algorithm select",
      async () => {
        render(<StructuredTab/>);
        const input = screen.getByTestId("file-input");
        await act(async () =>
    ...
    test("lists dropped files and triggers create callback",
      async () => {
        render(<StructuredTab/>);
        const input = screen.getByTestId("file-input");
        await act(async () =>

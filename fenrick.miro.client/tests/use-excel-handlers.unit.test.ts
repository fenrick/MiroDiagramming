...
vi.mock("react-dropzone",
  () => ({
    useDropzone: (opts: { onDrop: (files: File[]) => Promise<void> }) => {
      ...
      return { isDragAccept: false, isDragReject: false } as ReturnType<
        typeof useDropzone
      >;
    },
  }));
...
describe("use-excel-handlers helpers",
  () => {
    test("handleLocalDrop delegates to excel loader",
      async () => {
    ...
    test("fetchRemoteWorkbook delegates to graph loader",
      async () => {
        await fetchRemoteWorkbook("url");
        expect(graphExcelLoader.loadWorkbookFromGraph)
          .toHaveBeenCalledWith("url");
      });

    test("useExcelDrop forwards drop callback",
      async () => {
...
describe("use-excel-create",
  () => {
    test("creates nodes and updates rows",
      async () => {
        const processSpy = vi
    ...
    test("reports errors",
      async () => {
        vi.spyOn(GraphProcessor.prototype, "processGraph").mockRejectedValue(
          new Error("fail"),
        );
        const setRows = vi.fn();
        const { result } = renderHook(() =>

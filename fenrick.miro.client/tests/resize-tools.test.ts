...
describe("resize-tools",
  () => {
    ...
    test("copySizeFromSelection returns size",
      async () => {
        const board = {
    ...
    test("applySizeToSelection updates widgets",
      async () => {
    ...
    test("applySizeToSelection updates frames",
      async () => {
    ...
    test("applySizeToSelection skips unsupported items",
      async () => {
        const items = [
          { width: 1, height: 1, sync: jest.fn() }, { foo: "bar" }
        ];
    ...
    test("copySizeFromSelection returns null when invalid",
      async () => {
    ...
    test("applySizeToSelection throws without board",
      async () =>
      await expect(applySizeToSelection({ width: 1, height: 1 })).rejects
      .toThrow(
        "Miro board not available",
      ));

    test("scaleSelection multiplies widget dimensions",
      async () => {
    ...
    test("scaleSelection ignores unsupported items",
      async () => {
        const items = [
          { width: 4, height: 4, sync: jest.fn() }, { foo: "bar" }
        ];
        const board = { getSelection: jest.fn().mockResolvedValue(items) };

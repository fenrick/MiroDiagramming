...
describe("loadCards",
  () => {
    ...
    test("getInstance creates singleton when missing",
      () => {
        const original = (CardLoader as unknown as { instance?: CardLoader })
          .instance;
        (CardLoader as unknown as { instance?: CardLoader }).instance =
          undefined;
    ...
    test("throws on invalid file object",
      async () =>
      await expect(cardLoader.loadCards(null as unknown as File)).rejects
      .toThrow(
        "Invalid file",
      ));
    ...
    test("rejects when file load fails",
      async () => {
        class FR {
          onload: ((e: ReaderEvent) => void) | null = null;
          onerror: (() => void) | null = null;

...
describe("StyleTab extra features",
  () => {
    beforeEach(() => {
      ...
      (styleTools.copyFillFromSelection as unknown as vi.Mock)
        .mockResolvedValue(
          "#123456",
        );
    });

    test("opacity and border buttons call helpers",
      async () => {
    ...
    test("copy fill button updates preview",
      async () => {
        render(React.createElement(StyleTab));
        await act(async () =>
    ...
    test("apply button sends adjust delta to tweakFillColor",
      async () => {
    ...
    test("preset button applies corresponding style",
      async () => {

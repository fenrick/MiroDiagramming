...
from;
"../src/core/utils/excel-loader";
{
  ColumnMapping
}
from;
"../src/core/data-mapper";
{
  Group
}
from;
  ...
  () => {
    ...
    test("updateRowFromWidget writes label when provided",
      () => {
        ...
        const mapping =
          { idColumn: "ID", labelColumn: "Name" } as ColumnMapping;
        const updated =
          svc.updateRowFromWidget(row, mapping, { content: "New" });
    ...
    test("extractItem returns first group item",
      async () => {
        const svc = new ExcelSyncService() as unknown as {
          extractItem: (w: {
            type: string;
    ...
    test("extractItem returns widget for non group",
      async () => {
        const svc = new ExcelSyncService() as unknown as {
    ...
    test("findWidget uses cached id mapping",
      async () => {
    ...
    test("findWidget searches shapes then groups",
      async () => {
    ...
    test("pushChangesToExcel merges widget data",
      async () => {
        const svc = new ExcelSyncService() as unknown as {
          pushChangesToExcel: (
            rows: ExcelRow[],
        ...
        expect((svc as unknown as ExcelSyncService).getWidgetId("0"))
          .toBe("w0");
      });

    test("applyTemplate updates group content",
      async () => {
        ...
        const svc = new ExcelSyncService() as unknown as {
          applyTemplate: (w: Group, label: string, tpl: string) =>
          Promise<void>;
        };

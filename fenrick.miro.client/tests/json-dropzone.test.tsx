...
test("invokes callback when file selected",
  async () => {
    const handle = vi.fn();
    ...
    await act(
      async () => fireEvent.change(input, { target: { files: [file] } }));
    expect(handle).toHaveBeenCalled();
    expect(handle.mock.calls[0][0]).toEqual([file]);
  });

test("drag accept toggles text and triggers drop",
  async () => {
    const handle = vi.fn();
    render(<JsonDropZone onFiles={handle}/>);
    ...
    const data = {
      files: [file],
      items: [
        { kind: "file", type: "application/json", getAsFile: () => file }
      ],
      types: ["Files"],
    } as DataTransfer;
    await act(async () => {
...
test("drag reject toggles reject style",
  async () => {

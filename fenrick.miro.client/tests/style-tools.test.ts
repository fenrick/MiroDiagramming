...
describe("style-tools",
  () => {
    ...
    test("tweakFillColor adjusts fill and font",
      async () => {
        const item = {
    ...
    test("tweakFillColor updates frame styles",
      async () => {
        const item = {
    ...
    test("tweakFillColor respects textColor",
      async () => {
        const item = {
    ...
    test("tweakFillColor supports backgroundColor",
      async () => {
        const item = {
    ...
    test("tweakFillColor skips unsupported items",
      async () => {
        const items = [
    ...
    test("tweakFillColor throws without board",
      async () =>
      await expect(tweakFillColor(0.1)).rejects.toThrow(
        "Miro board not available",
      ));

    test("copyFillFromSelection returns colour",
      async () => {
        const board = {
          getSelection: jest
            .fn()
    ...
    test("copyFillFromSelection returns null when missing",
      async () => {
    ...
    test("tweakOpacity adjusts fillOpacity",
      async () => {
    ...
    test("tweakOpacity clamps between 0 and 1",
      async () => {
    ...
    test("tweakOpacity skips unsupported items",
      async () => {
        const items = [
          { style: { opacity: 0.5 }, sync: jest.fn() }, { foo: 1 }
        ];
        const board = { getSelection: jest.fn().mockResolvedValue(items) };
    ...
    test("tweakBorderWidth updates border style",
      async () => {
    ...
    test("tweakBorderWidth handles strokeWidth",
      async () => {
    ...
    test("tweakBorderWidth skips unsupported items",
      async () => {
        const items = [
          { style: { lineWidth: 1 }, sync: jest.fn() }, { bar: 2 }
        ];
        const board = { getSelection: jest.fn().mockResolvedValue(items) };
        await tweakBorderWidth(1, board);

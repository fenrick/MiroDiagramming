...
describe("undoWidgets",
  () => {
    test("removes items when registry populated",
      async () => {
    ...
    test("skips removal when registry empty",
      async () => {
        const builder = { removeItems: jest.fn() } as unknown as BoardBuilder;
...
describe("syncOrUndo",
  () => {
    test("rolls back when sync fails",
      async () => {
        const builder = {
    ...
    test("leaves items intact when sync succeeds",
      async () => {
        const builder = {
          syncAll: jest.fn(),
          removeItems: jest.fn(),

...
describe("spacing-tools",
  () => {
    ...
    test("applySpacingLayout spaces widgets horizontally by edges",
      async () => {
        const items = [
    ...
    test("applySpacingLayout grows widgets horizontally",
      async () => {
        const items = [
          { x: 0, y: 0, width: 10, sync: jest.fn() },
        ...
        await applySpacingLayout({ axis: "x", spacing: 5, mode: "grow" },
          board);
        expect(items.map(i => i.width)).toEqual([20, 20, 20]);
        expect(items.map(i => i.x)).toEqual([5, 30, 55]);
      });

    test("applySpacingLayout grows widgets vertically",
      async () => {
        const items = [
        ...
        await applySpacingLayout({ axis: "y", spacing: 5, mode: "grow" },
          board);
        expect(items.map(i => i.height)).toEqual([20, 20]);
        expect(items.map(i => i.y)).toEqual([5, 30]);
      });

    test("applySpacingLayout handles frames",
      async () => {
        const items = [
    ...
    test("applySpacingLayout skips unsupported items",
      async () => {
        const items = [
          { x: 0, y: 0, width: 10, sync: jest.fn() }, { foo: "bar" }
        ];
        const board: BoardLike = {
          getSelection: jest.fn().mockResolvedValue(items),
        };
    ...
    test("applySpacingLayout spaces widgets vertically by edges",
      async () => {
        const items = [
          { x: 0, y: 0, height: 10, sync: jest.fn() },
    ...
    test("applySpacingLayout returns early on empty selection",
      async () => {
        const board: BoardLike =
    ...
    test("applySpacingLayout throws without board",
      async () =>
      await expect(applySpacingLayout({ axis: "x", spacing: 1 })).rejects
      .toThrow(

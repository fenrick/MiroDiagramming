...
describe("BoardBuilder resizeItem",
  () => {
    test("updates width and height without syncing",
      async () => {
        const builder = new BoardBuilder();
        const item =
          { width: 1, height: 2, sync: vi.fn() } as Partial<BaseItem> & {
            sync: vi.Mock;
          };
    ...
    test("createNode applies layout size",
      async () => {
        const builder = new BoardBuilder();
    ...
    test("groupItems groups widgets together",
      async () => {
        const builder = new BoardBuilder();

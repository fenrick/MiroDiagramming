...
describe("excel loader",
  () => {
    ...
    test("loads workbook and lists sheets",
      async () => {
    ...
    test("loads rows from a named table",
      async () => {
        const loader = new ExcelLoader();
        await loader.loadWorkbook(file);
    ...
    test("validateFile catches invalid objects",
      async () => {
        const loader = new ExcelLoader();
        const invalid = {
          async arrayBuffer() {
        ...
        await expect(loader.loadWorkbook(invalid)).rejects
          .toThrow("Invalid file");
      });

    test("loads workbook from fixture file",
      async () => {
        const loader = new ExcelLoader();
    ...
    test("loadNamedTable throws on missing sheet reference",
      async () => {
        const loader = new ExcelLoader();
        await loader.loadWorkbook(file);
        const wb = (loader as unknown as { workbook: ExcelJS.Workbook })
          .workbook!;
    ...
    test("parseRange rejects malformed references",
      async () => {
        const loader = new ExcelLoader();
        await loader.loadWorkbook(file);
        ...
        expect(() => helper.parseRange("A1B2", ws))
          .toThrow("Invalid range: A1B2");
      });
  });

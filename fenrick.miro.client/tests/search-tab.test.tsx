...
describe("SearchTab",
  () => {
    ...
    test("debounced search updates match count",
      async () => {
    ...
    test("bulk replace calls replaceBoardContent and focuses matches",
      async () => {
        ...
        await act(
          async () => fireEvent.click(screen.getByText(/replace all/i)));
        ...
        expect(screen.getByTestId("match-count"))
          .toHaveTextContent("Matches: 0");
      });

    test("filters are passed to search utilities",
      async () => {
        const searchSpy = vi
    ...
    test("next button zooms to each match",
      async () => {
        const results = [
          { item: { id: "a" }, field: "t" },
          { item: { id: "b" }, field: "t" },
        ...
        await act(async () => fireEvent.click(screen.getByText(/next/i)));
        expect(global.miro.board.viewport.zoomToObject)
          .toHaveBeenLastCalledWith(
            results[0].item,
          );
      });

    test("replace button updates single match and focuses it",
      async () => {
    ...
    test("clearing query resets matches without new search",
      async () => {
        const spy = vi
          .spyOn(searchTools, "searchBoardContent")
    ...
    test("zoomTo used when zoomToObject unavailable",
      async () => {
        const results = [
          { item: { id: "a" }, field: "t" },
          { item: { id: "b" }, field: "t" },
    ...
    test("next handles missing viewport gracefully",
      async () => {
        const results = [
          { item: { id: "x" }, field: "t" },
          { item: { id: "y" }, field: "t" },
        ...
        await act(async () => fireEvent.click(screen.getByText(/next/i)));
        expect(screen.getByTestId("match-count"))
          .toHaveTextContent("Matches: 2");
      });

    test("widget type filter toggles off",
      async () => {
        const searchSpy = vi
          .spyOn(searchTools, "searchBoardContent")
          .mockResolvedValue([]);
    ...
    test("replace all ignored when query is empty",
      async () => {

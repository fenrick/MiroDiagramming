...
test("layoutGraph handles metadata and missing sections",
  async () => {
...
test("layoutGraph uses defaults when layout values missing",
  async () => {
    const layoutSpy = jest
      .spyOn(ELK.prototype, "layout")
...
test("layoutGraph uses template dimensions when metadata absent",
  async () => {
    const spy = jest
      .spyOn(ELK.prototype, "layout")
      .mockImplementation(async (g: unknown) => {
...
test("layoutGraph handles missing edge sections array",
  async () => {
    jest
      .spyOn(ELK.prototype, "layout")

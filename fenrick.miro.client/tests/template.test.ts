...
describe("createFromTemplate",
  () => {
    ...
    test("creates a single shape with correct style",
      async () => {
        ...
        const args =
          (global.miro.board.createShape as jest.Mock).mock.calls[0][0];
    ...
    test("groups multiple elements",
      async () => {
        ...
        const items = (global.miro.board.group as jest.Mock).mock.calls[0][0]
          .items;
        expect(items).toHaveLength(2);
      });

    test("adds created items to provided frame",
      async () => {
        (
          templateManager as unknown as { templates: Record<string, unknown> }
        ).templates.withFrame = {
        ...
        import("@mirohq/websdk-types").Frame;
        const widget = await templateManager.createFromTemplate(
          "withFrame",
          "L",
          0,
    ...
    test("single element added to frame without grouping",
      async () => {
        (
          templateManager as unknown as { templates: Record<string, unknown> }
    ...
    test("apply fill property when style lacks fillColor",
      async () => {
        (
          templateManager as unknown as { templates: Record<string, unknown> }
        ).templates.fillStyle = {

...
describe("GraphClient",
  () => {
    ...
    test("builds url from share link",
      async () => {
        (fetch as unknown as vi.Mock).mockResolvedValue({
          ok: true,
          arrayBuffer: () => Promise.resolve(new ArrayBuffer(1)),
    ...
    test("encodes unicode share link",
      async () => {
        (fetch as unknown as vi.Mock).mockResolvedValue({
          ok: true,
          arrayBuffer: () => Promise.resolve(new ArrayBuffer(1)),
        ...
        expect((fetch as vi.Mock).mock.calls[0][0]).toBe(
          `https://graph.microsoft.com/v1.0/shares/u!${encoded
          }/driveItem/content`,
        );
        (globalThis as { btoa?: (s: string) => string }).btoa = originalBtoa;
      });

    test("encodes special characters in share link",
      async () => {
        (fetch as unknown as vi.Mock).mockResolvedValue({
          ok: true,
          arrayBuffer: () => Promise.resolve(new ArrayBuffer(1)),

...
describe("buildShapeStyle",
  () => {
    ...
    test("merges existing style with template style",
      () => {
        vi.spyOn(templateManager, "resolveStyle").mockImplementation(
          style => style,
        );
...
describe("applyShapeElement and applyTextElement",
  () => {
    ...
    test("updates shape geometry and content",
      () => {
        vi.spyOn(templateManager, "resolveStyle")
          .mockReturnValue({ color: "red" });
    ...
    test("applyElementToItem routes by type",
      () => {
        ...
        applyElementToItem(
          shape as BaseItem,
          { text: "A" } as TemplateElement,
          "x",
        );
        applyElementToItem(text as BaseItem,
          { text: "B" } as TemplateElement,
          "y");

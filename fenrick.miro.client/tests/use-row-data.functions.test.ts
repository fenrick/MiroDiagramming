...
describe("use-row-data helpers",
  () => {
    test("extractRowId returns first child value for groups",
      async () => {
        const widget = {
          type: "group",
          getItems: vi.fn()
            .mockResolvedValue([{ content: "1" }, { content: "2" }]),
        };
        await expect(
          extractRowId(widget as unknown as BaseItem | Group),
        ).resolves.toBe("1");
      });

    test("extractRowId handles single widget",
      async () => {

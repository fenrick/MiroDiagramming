...
export const ExcelTab: React.FC = () => {
  ...
  const [diff, setDiff] = React.useState<DiffResult<NodeDefinition> | null>(null)
  const [jobId, setJobId] = React.useState('')
  const boardId = ((globalThis as { miro?: { board?: { info?: { id: string } } } }).miro?.board
    ?.info?.id ?? '') as string
  ...
  return (
    <>
      ...
      <JobDrawer jobId={jobId} isOpen={jobId !== ''} onClose={() => setJobId('')} />
    </>
  )
}

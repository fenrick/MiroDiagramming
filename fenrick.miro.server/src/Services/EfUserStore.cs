...
public class EfUserStore(MiroDbContext context) : IUserStore
{
    ...
    /// <inheritdoc />
    public async Task<UserInfo?> RetrieveAsync(string userId,
        CancellationToken ct = default)
    {
        ...
        UserEntity? entity = await this.db.Users.FindAsync([userId], ct)
    ...
    public async Task StoreAsync(UserInfo info, CancellationToken ct = default)
    {
        ...
        UserEntity? entity = await this.db.Users.FindAsync([info.Id], ct)
            .ConfigureAwait(false);
        if (entity is null)
        {
            await this.db.Users

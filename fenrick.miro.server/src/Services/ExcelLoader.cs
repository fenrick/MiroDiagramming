...
public class ExcelLoader(ILogger<ExcelLoader>? log = null) : IDisposable
{
    private readonly ILogger<ExcelLoader> logger =
        log ?? NullLogger<ExcelLoader>.Instance;
    ...
    private static class Log
    {
        public static readonly Action<ILogger, Exception?> LoadingWorkbook =
            LoggerMessage.Define(LogLevel.Debug,
                new EventId(1, nameof(LoadAsync)), LogMessages.LoadingWorkbook);

        public static readonly Action<ILogger, int, Exception?> WorkbookLoaded =
            LoggerMessage.Define<int>(LogLevel.Debug,
                new EventId(2, nameof(LoadAsync)), LogMessages.WorkbookLoaded);

        public static readonly Action<ILogger, Exception?> ListingSheets =
            LoggerMessage.Define(LogLevel.Trace,
                new EventId(3, nameof(ListSheets)), LogMessages.ListingSheets);

        public static readonly Action<ILogger, Exception?> ListingTables =
            LoggerMessage.Define(LogLevel.Trace,
                new EventId(4, nameof(ListNamedTables)),
                LogMessages.ListingTables);

        public static readonly Action<ILogger, string, Exception?>
            LoadingSheet =
                LoggerMessage.Define<string>(LogLevel.Debug,
                    new EventId(5, nameof(LoadSheet)),
                    LogMessages.LoadingSheet);

        public static readonly Action<ILogger, int, Exception?> LoadedRows =
            LoggerMessage.Define<int>(LogLevel.Debug,
                new EventId(6, nameof(LoadSheet)), $"Loaded {{RowCount}} rows");

        public static readonly Action<ILogger, string, Exception?>
            StreamingSheet =
                LoggerMessage.Define<string>(LogLevel.Debug,
                    new EventId(7, nameof(StreamSheetAsync)),
                    LogMessages.StreamingSheet);

        public static readonly Action<ILogger, string, Exception?>
            StreamingTable =
                LoggerMessage.Define<string>(LogLevel.Debug,
                    new EventId(8, nameof(StreamNamedTableAsync)),
                    LogMessages.StreamingTable);

        public static readonly Action<ILogger, string, Exception?>
            LoadingTable =
                LoggerMessage.Define<string>(LogLevel.Debug,
                    new EventId(9, nameof(LoadNamedTable)),
                    LogMessages.LoadingTable);

        public static readonly Action<ILogger, Exception?> WorkbookNotLoaded =
            LoggerMessage.Define(LogLevel.Error,
                new EventId(10, nameof(ExcelLoader)),
                LogMessages.WorkbookNotLoaded);

        public static readonly Action<ILogger, string, Exception?>
            UnknownSheet =
                LoggerMessage.Define<string>(LogLevel.Error,
                    new EventId(11, nameof(ExcelLoader)),
                    LogMessages.UnknownSheet);

        public static readonly Action<ILogger, string, Exception?>
            UnknownTable =
                LoggerMessage.Define<string>(LogLevel.Error,
                    new EventId(12, nameof(ExcelLoader)),
                    LogMessages.UnknownTable);

        public static readonly Action<ILogger, string, Exception?>
            MissingSheetForTable =
                LoggerMessage.Define<string>(LogLevel.Error,
                    new EventId(13, nameof(ExcelLoader)),
                    LogMessages.MissingSheetForTable);

        public static readonly Action<ILogger, Exception?> DisposingWorkbook =
            LoggerMessage.Define(LogLevel.Trace,
                new EventId(14, nameof(Dispose)),
                LogMessages.DisposingWorkbook);
    }
    ...
    public async Task LoadAsync(Stream stream)
    {
        ...
        try
        {
            ...
            Log.WorkbookLoaded(this.logger, this.workbook.Worksheets.Count,
                null);
    ...
    public IReadOnlyList<Dictionary<string, string>> LoadSheet(string name)
    {
        ...
        {
            var ex =
                new InvalidOperationException(LogMessages.WorkbookNotLoaded);
            Log.WorkbookNotLoaded(this.logger, ex);
            throw ex;
        }
    ...
    public IAsyncEnumerable<Dictionary<string, string>> StreamNamedTableAsync(
        string name)
    {
        ...
        if (!this.workbook.DefinedNames.TryGetValue(name,
                out IXLDefinedName? def) || def.Ranges.Count == 0)
        {
            var ex = new ArgumentException(
        ...
        async IAsyncEnumerable<Dictionary<string, string>>
            StreamNamedTableAsync()
        {
            var headers = range.FirstRow().Cells().Select(c => c.GetString())
                .ToList();

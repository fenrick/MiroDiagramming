    ...
    : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> CreateAsync(string boardId,
        [FromBody] ShapeData[] shapes)
    {
        var responses =
            await this.miroClient.CreateAsync($"/boards/{boardId}/shapes",
                shapes);
        for (var i = 0; i < responses.Count && i < shapes.Length; i++)
        {
            this.shapeCache.Store(new ShapeCacheEntry(boardId,
                responses[i].Body, shapes[i]));
        }
    ...
    [HttpPut("{itemId}")]
    public async Task<IActionResult> UpdateAsync(string boardId, string itemId,
        [FromBody] ShapeData shape)
    {
        var body = JsonSerializer.Serialize(shape);
        var response = await this.miroClient.SendAsync(
            new MiroRequest("PUT", $"/boards/{boardId}/shapes/{itemId}", body));
    ...
    {
        var response = await this.miroClient.SendAsync(new MiroRequest("DELETE",
            $"/boards/{boardId}/shapes/{itemId}", null));

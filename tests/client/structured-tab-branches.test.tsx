...
vi.mock('../src/ui/hooks/ui-utils', async () => {
  const actual = await vi.importActual<typeof import('../src/ui/hooks/ui-utils')>(
...
describe('StructuredTab branches', () =>
  test('shows progress, error and undo workflow', async () => {
    ...
    await act(async () => fireEvent.change(input, { target: { files: [makeFile('graph.json')] } }))
    ...
    expect(screen.queryByRole('button', { name: /undo last import/i })).not.toBeInTheDocument()
  }))

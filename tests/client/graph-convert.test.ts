...
describe('processor conversions', () => {
  ...
  test('GraphProcessor converts hierarchy input', async () => {
    ...
    const spy = vi.spyOn(layoutEngine, 'layoutGraph').mockResolvedValue({
      nodes: {
    ...
    await gp.processGraph(hierarchy as unknown as Parameters<typeof gp.processGraph>[0])
  ...
  test('HierarchyProcessor converts graph input', async () => {
    ...
    const spy = vi.spyOn(nestedLayout, 'layoutHierarchy').mockResolvedValue({
      nodes: {
        p: { x: 0, y: 0, width: 10, height: 10 },
        c: { x: 10, y: 0, width: 10, height: 10 },
      },
    } as ReturnType<typeof nestedLayout.layoutHierarchy>)
    await proc.processHierarchy(graph as unknown as Parameters<typeof proc.processHierarchy>[0])
    expect(spy).toHaveBeenCalled()
  })
})

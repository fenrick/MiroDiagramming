...
vi.mock('@mirohq/design-system', async () => {
  const actual =
    await vi.importActual<typeof import('@mirohq/design-system')>('@mirohq/design-system')
  const Slider = Object.assign((props: React.PropsWithChildren) => <div {...props} />, {
...
describe('StyleTab extra features', () => {
  beforeEach(() => {
    ;(styleTools.tweakOpacity as unknown as vi.Mock).mockResolvedValue(undefined)
    ;(styleTools.tweakBorderWidth as unknown as vi.Mock).mockResolvedValue(undefined)
    ;(styleTools.copyFillFromSelection as unknown as vi.Mock).mockResolvedValue('#123456')
  })
  ...
  test('copy fill button updates preview', async () => {
    render(React.createElement(StyleTab))
    await act(async () => fireEvent.click(screen.getByRole('button', { name: /copy fill/i })))
    expect(screen.getByTestId('color-hex')).toHaveTextContent('#123456')
  })

  test('apply button sends adjust delta to tweakFillColor', async () => {
    ;(styleTools.tweakFillColor as unknown as vi.Mock).mockResolvedValue(undefined)
    render(React.createElement(StyleTab))
  ...
  test('preset button applies corresponding style', async () => {
    ...
    expect(formatTools.applyStylePreset).toHaveBeenCalledWith(stylePresets[name])
  })
})

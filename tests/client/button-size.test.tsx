...
it('defaults to large size for primary', () => {
  ...
  expect(getByRole('button').className).toEqual(expect.stringContaining('size-large'))
})

it('defaults to medium size for secondary', () => {
  const { getByRole } = render(<Button variant="secondary">Ok</Button>)
  expect(getByRole('button').className).toEqual(expect.stringContaining('size-medium'))
})

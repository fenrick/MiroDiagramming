...
describe('loadCards', () => {
  ...
  test('getInstance creates singleton when missing', () => {
    const original = (CardLoader as unknown as { instance?: CardLoader }).instance
  ...
  test('throws on invalid file object', async () =>
    await expect(cardLoader.loadCards(null as unknown as File)).rejects.toThrow('Invalid file'))
  ...
  test('throws on invalid data', async () => {
    ...
    ;(global as { FileReader?: unknown }).FileReader = FR
    await expect(cardLoader.loadCards({ name: 'x.json' } as unknown as File)).rejects.toThrow(
      'Invalid card data',
    )
  })
  ...
  test('rejects when file load fails', async () => {
    ...
    ;(global as { FileReader?: unknown }).FileReader = FR
    await expect(cardLoader.loadCards({ name: 'bad.json' } as unknown as File)).rejects.toEqual(

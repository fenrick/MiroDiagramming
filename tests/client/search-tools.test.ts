...
describe('search-tools', () => {
  ...
  test('widget type filter', async () => {
    const { board, items } = makeBoard()
    const res = await searchBoardContent({ query: 'hello', widgetTypes: ['card'] }, board)
  ...
  test('tag, colour and assignee filters', async () => {
    const { board, items } = makeBoard()
    const byTag = await searchBoardContent({ query: 'foo', tagIds: ['t2'] }, board)
    expect(byTag).toHaveLength(1)
    expect(byTag[0].item).toBe(items[2])
    const byColour = await searchBoardContent({ query: 'hello', backgroundColor: '#fff' }, board)
    expect(byColour.map((r) => r.item)).toEqual(expect.arrayContaining([items[0], items[4]]))
    const byAssignee = await searchBoardContent({ query: 'hello', assignee: 'u2' }, board)
  ...
  test('creator and modifier filters', async () => {
    const { board, items } = makeBoard()
    const byCreator = await searchBoardContent({ query: 'hello', creator: 'c4' }, board)
    expect(byCreator).toHaveLength(1)
    expect(byCreator[0].item).toBe(items[5])
    const byModifier = await searchBoardContent({ query: 'hello', lastModifiedBy: 'm1' }, board)
    expect(byModifier.map((r) => r.item)).toEqual(expect.arrayContaining([items[0], items[5]]))
  })
  ...
  test('regex and whole-word options', async () => {
    ...
    expect(regex.map((r) => r.item)).toEqual(expect.arrayContaining([items[0], items[3], items[5]]))
  ...
  test('case sensitivity and selection only', async () => {
    ...
    const sel = await searchBoardContent({ query: 'hello', inSelection: true }, board)
    expect(board.getSelection).toHaveBeenCalled()
    expect(board.get).not.toHaveBeenCalled()
    expect(sel.map((r) => r.item)).toEqual(expect.arrayContaining([items[0], items[1]]))
  })
  ...
  test('onMatch callback is invoked for each replacement', async () => {
    const { board, items } = makeBoard()
    const seen: unknown[] = []
    const count = await replaceBoardContent({ query: 'hello', replacement: 'hi' }, board, (i) =>
      seen.push(i),
    )
    expect(count).toBe(4)
    expect(seen).toEqual(expect.arrayContaining([items[0], items[1], items[4], items[5]]))
  })
  ...
  test('search utilities throw without board', async () => {
    await expect(searchBoardContent({ query: 'hi' })).rejects.toThrow('Miro board not available')
    await expect(replaceBoardContent({ query: 'hi', replacement: 'bye' })).rejects.toThrow(
  ...
  test('searchBoardContent rejects invalid regex patterns', async () => {
    ...
    await expect(searchBoardContent({ query: '*foo', regex: true }, board)).rejects.toThrow(
      SyntaxError,
    )
  })
  ...
  test('replaceBoardContent rejects invalid regex patterns', async () => {
    const { board } = makeBoard()
    await expect(
      replaceBoardContent({ query: '*foo', replacement: 'bar', regex: true }, board),
    ).rejects.toThrow(SyntaxError)
  })

  test('searchBoardContent rejects unsafe regex patterns', async () => {
    const { board } = makeBoard()
    await expect(searchBoardContent({ query: '(aa+)$', regex: true }, board)).rejects.toThrow(
  ...
  test('replaceBoardContent rejects excessively long patterns', async () => {
    ...
    await expect(
      replaceBoardContent({ query: pattern, replacement: 'x', regex: true }, board),

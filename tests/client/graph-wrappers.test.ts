...
describe('graph service methods', () => {
  ...
  test('findNode delegates to default builder', async () => {
    const spy = vi.spyOn(defaultBuilder, 'findNode').mockResolvedValue('x' as unknown)
  ...
  test('createNode delegates to default builder', async () => {
    const spy = vi.spyOn(defaultBuilder, 'createNode').mockResolvedValue('n' as unknown)
    const result = await graphService.createNode({} as Record<string, unknown>, {
      x: 0,
      y: 0,
      width: 1,
      height: 1,

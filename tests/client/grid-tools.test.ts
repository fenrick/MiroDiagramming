...
describe('grid-tools', () => {
  ...
  test('calculateGridPositions computes offsets', () => {
    ...
    const positions = calculateGridPositions({ cols: 2, padding: 5 }, 4, 10, 10)
  ...
  test('applyGridLayout positions widgets', async () => {
    ...
    await applyGridLayout({ cols: 1, padding: 5, sortByName: true, groupResult: true }, board)
  ...
  test('applyGridLayout sorts by nested text fields', async () => {
    ...
    await applyGridLayout({ cols: 1, padding: 0, sortByName: true, groupResult: false }, board)
  ...
  test('applyGridLayout ignores unsupported items', async () => {
    const items = [{ x: 0, y: 0, width: 10, height: 10, sync: vi.fn() }, { foo: 'bar' }]
    const board: BoardLike = { getSelection: vi.fn().mockResolvedValue(items) }

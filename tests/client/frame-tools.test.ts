...
import { lockSelectedFrames, renameSelectedFrames } from '../src/board/frame-tools'
...
describe('frame-tools', () => {
  ...
  test('renameSelectedFrames throws without board', async () =>
    await expect(renameSelectedFrames({ prefix: 'P' })).rejects.toThrow('Miro board not available'))
  ...
  describe('lockSelectedFrames', () => {
    ...
    test('throws without board', async () =>
      await expect(lockSelectedFrames()).rejects.toThrow('Miro board not available'))
  })
})

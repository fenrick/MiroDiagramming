...
describe('GraphProcessor.processFile', () => {
  ...
  test('throws on invalid file', async () => {
    ...
    await expect(gp.processFile(null as unknown as File)).rejects.toThrow('Invalid file')
  })

  test('loads graph then processes it', async () => {
    const gp = new GraphProcessor()
    const mockGraph = { nodes: [], edges: [] } as Parameters<GraphProcessor['processGraph']>[0]

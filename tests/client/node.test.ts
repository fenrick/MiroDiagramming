...
describe('createNode', () => {
  beforeEach(() => {
    ...
    vi.spyOn(templateManager, 'createFromTemplate').mockResolvedValue({
      ...
      sync: vi.fn(),
      id: 's1',
    } as unknown as { type: string; setMetadata: vi.Mock } & Record<string, unknown>)
  })
  ...
  const node = { id: 'n1', label: 'L', type: 'Motivation' } as Record<string, unknown>
  ...
  test('ignores existing node', async () => {
    const existing = {
      ...
      style: {},
      setMetadata: vi.fn(),
      getMetadata: vi.fn().mockResolvedValue({ type: 'Motivation', label: 'L' }),
      sync: vi.fn(),
      id: 'sExisting',

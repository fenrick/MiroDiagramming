...
test('layoutGraph handles metadata and missing sections', async () => {
  const layoutSpy = vi.spyOn(ELK.prototype, 'layout').mockImplementation(async (g: unknown) => {
...
test('layoutGraph uses template dimensions when metadata absent', async () => {
  const spy = vi.spyOn(ELK.prototype, 'layout').mockImplementation(async (g: unknown) => {
  ...
  await layoutEngine.layoutGraph(graph as unknown as Parameters<typeof layoutEngine.layoutGraph>[0])
  spy.mockRestore()
})

...
describe('SyncStatusBar', () => {
  ...
  test('shows near limit warning', async () => {
    ...
    render(<SyncStatusBar />)
    expect(await screen.findByText('Slowing to avoid limits')).toBeInTheDocument()
  })
  ...
  test('shows rate limited when bucket empty', async () => {
    ...
    render(<SyncStatusBar />)
    expect(await screen.findByText('Paused for 12s (auto-resume)')).toBeInTheDocument()
    await act(async () => {
      vi.advanceTimersByTime(1000)
    })
    expect(await screen.findByText('Paused for 11s (auto-resume)')).toBeInTheDocument()
    vi.useRealTimers()
  })

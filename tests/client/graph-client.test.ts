...
describe('GraphClient', () => {
  ...
  test('builds url from id', async () => {
    ...
    expect((fetch as vi.Mock).mock.calls[0][0]).toMatch(/me\/drive\/items\/123\/content/)
    expect((fetch as vi.Mock).mock.calls[0][1].headers.Authorization).toBe('Bearer token')
  })
  ...
  test('throws on error', async () => {
    ...
    await expect(client.fetchFile('id')).rejects.toThrow('Failed to fetch workbook')
  })
})

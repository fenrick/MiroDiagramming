...
describe('loadGraph', () => {
  ...
  test('throws on invalid file object', async () => {
    ...
    await expect(graphService.loadGraph(null as unknown as File)).rejects.toThrow('Invalid file')
  })
  ...
  test('throws on invalid graph data', async () => {
    ...
    ;(global as { FileReader?: unknown }).FileReader = FR
    await expect(graphService.loadGraph({ name: 'a.json' } as unknown as File)).rejects.toThrow(
      'Invalid graph data',
    )
  })
  ...
  test('rejects when FileReader has no target', async () => {
    ...
    ;(global as { FileReader?: unknown }).FileReader = FR
    await expect(graphService.loadGraph({ name: 'bad.json' } as unknown as File)).rejects.toEqual(

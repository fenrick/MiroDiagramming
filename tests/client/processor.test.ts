...
describe('GraphProcessor', () => {
  ...
  it('throws on invalid graph', async () =>
    await expect(processor.processGraph({} as unknown)).rejects.toThrow('Invalid graph format'))
  ...
  it('positions frame at space center', async () => {
    ...
    const createArgs = (global.miro.board.createFrame as vi.Mock).mock.calls[0][0]
  ...
  it('records widget ids for rows', async () => {
    const simpleGraph = {
      nodes: [{ id: 'n1', label: 'A', type: 'Motivation', metadata: { rowId: 'r1' } }],
      edges: [],
    }
    vi.spyOn(layoutEngine, 'layoutGraph').mockResolvedValue({
      nodes: { n1: { x: 0, y: 0, width: 10, height: 10 } },

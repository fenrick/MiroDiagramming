...
describe('BoardBuilder lookup and connector updates', () => {
  ...
  test('findNodeInSelection uses cached selection', async () => {
    ...
    await builder.findNodeInSelection('Business', 'B')
    expect((global.miro.board.getSelection as vi.Mock).mock.calls.length).toBe(1)
  })
  ...
  test('createEdges skips connector lookup', async () => {
    ...
    const nodeMap = { n1: { id: 'a' }, n2: { id: 'b' } } as Record<string, unknown>

    await builder.createEdges(edges as unknown as Array<{ from: string; to: string }>, nodeMap)

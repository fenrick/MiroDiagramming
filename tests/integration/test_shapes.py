...
async def test_create_shape_enqueues_task(
    ...
    # Simulate application restart by using a new store instance.
    new_store = DbShapeStore(SessionLocal())
    persisted = new_store.get("b1", data["id"])
    Base.metadata.drop_all(bind=engine)
    assert persisted is not None
    assert persisted.content == "box"
